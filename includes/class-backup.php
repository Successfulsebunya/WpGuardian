...
class WPGuardian_Backup {
	...
	public static function get_backup_dir() {
		...
		return trailingslashit( $upload_dir['basedir'] ) . 'wp-guardian-backups/';
	}
	...
	public static function create_partial_backup( $post_id ) {
		...
		if ( ! $post ) {
			$error = new WP_Error( 'wpguardian_post_missing', __( 'Post not found.', 'wp-guardian' ) );
		...
		if ( false === $written ) {
			$error = new WP_Error( 'wpguardian_partial_write_failed', __( 'Could not write partial backup file.', 'wp-guardian' ) );
			self::notify_failure( $error->get_error_message(), 'partial' );
			return $error;
		}
	...
	private static function export_database_to_file( $target_file ) {
		...
		if ( empty( $tables ) ) {
			return new WP_Error( 'wpguardian_no_tables', __( 'No tables found to export.', 'wp-guardian' ) );
		}

		$handle = fopen( $target_file, 'wb' );
		if ( ! $handle ) {
			return new WP_Error( 'wpguardian_sql_write_failed', __( 'Could not open database backup file.', 'wp-guardian' ) );
		}

		$header = "-- WP Guardian SQL backup\n-- Generated: " . gmdate( 'c' ) . "\n\nSET FOREIGN_KEY_CHECKS=0;\n\n";
	...
	private static function zip_directory( $source_dir, $zip_path ) {
		if ( ! class_exists( 'ZipArchive' ) ) {
			return new WP_Error( 'wpguardian_zip_missing', __( 'ZipArchive extension is missing.', 'wp-guardian' ) );
		}

		$zip = new ZipArchive();
		if ( true !== $zip->open( $zip_path, ZipArchive::CREATE | ZipArchive::OVERWRITE ) ) {
			return new WP_Error( 'wpguardian_zip_open_failed', __( 'Unable to create backup archive.', 'wp-guardian' ) );
		}
	...
	private static function notify_failure( $message, $type ) {
		...
		$subject = sprintf( '[WP Guardian] %s backup failed', ucfirst( sanitize_text_field( $type ) ) );
